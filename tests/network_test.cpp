...
TEST_F(NetworkTest, transfer_lot_of_blocks) {
  ...
  for (auto trx : verified_transactions) node1->getTransactionManager()->insertValidatedTransaction(std::move(trx));
...
TEST_F(NetworkTest, node_sync) {
  ...
  for (size_t i = 0; i < blks.size(); ++i) {
    node1->getTransactionManager()->insertValidatedTransaction({blks[i].second, TransactionStatus::Verified});
...
TEST_F(NetworkTest, node_pbft_sync) {
  ...
  DagBlock blk1(dag_genesis, 1, {}, {g_signed_trx_samples[0]->getHash(), g_signed_trx_samples[1]->getHash()}, 0, vdf1,
                sk);
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[0], TransactionStatus::Verified});
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[1], TransactionStatus::Verified});
  ...
  DagBlock blk2(blk1.getHash(), 2, {}, {g_signed_trx_samples[2]->getHash(), g_signed_trx_samples[3]->getHash()}, 0,
                vdf2, sk);
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[2], TransactionStatus::Verified});
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[3], TransactionStatus::Verified});
...
TEST_F(NetworkTest, node_sync_with_transactions) {
  ...
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[4], TransactionStatus::Verified});
  ...
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[5], TransactionStatus::Verified});
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[6], TransactionStatus::Verified});
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[7], TransactionStatus::Verified});
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[8], TransactionStatus::Verified});
  ...
  node1->getTransactionManager()->insertValidatedTransaction({g_signed_trx_samples[9], TransactionStatus::Verified});
...
TEST_F(NetworkTest, node_sync2) {
  ...
  for (size_t i = 0; i < blks.size(); ++i) {
    for (auto t : trxs[i]) node1->getTransactionManager()->insertValidatedTransaction({t, TransactionStatus::Verified});
...
TEST_F(NetworkTest, node_transaction_sync) {
  ...
  for (auto t : *g_signed_trx_samples)
    node1->getTransactionManager()->insertValidatedTransaction({t, TransactionStatus::Verified});
...
TEST_F(NetworkTest, node_full_sync) {
  ...
  for (auto i = 0; i < num_of_trxs; ++i) {
    nodes[distNodes(rng)]->getTransactionManager()->insertValidatedTransaction({trxs[i], TransactionStatus::Verified});
