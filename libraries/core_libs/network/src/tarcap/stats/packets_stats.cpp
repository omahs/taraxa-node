...
void PacketsStats::addReceivedPacket(const std::string& packet_type, const SinglePacketStats& packet) {
  ...
  auto packet_json = packet.getStatsJson(true);
...
void PacketsStats::logAndUpdateStats() {
  ...
  LOG(log_nf_) << "Received packets stats: " << jsonToUnstyledString(period_received_packets_stats.getStatsJson(true));
  LOG(log_nf_) << "Sent packets stats: " << jsonToUnstyledString(period_sent_packets_stats.getStatsJson(false));
  ...
  LOG(log_dg_) << "Received packets period max count/size stats: "
               << jsonToUnstyledString(received_packets_stats_.getPeriodMaxStatsJson(false));
