...
std::pair<bool, std::string> TransactionManager::insertTransaction(const std::shared_ptr<Transaction> &trx) {
  ...
  if (insertValidatedTransaction({trx, status})) {
...
bool TransactionManager::insertValidatedTransaction(std::pair<std::shared_ptr<Transaction>, TransactionStatus> &&tx) {
  const auto trx_hash = tx.first->getHash();
  ...
  if (transactions_pool_.insert(std::move(tx), last_block_number)) {
    return true;
  }
  return false;
}
